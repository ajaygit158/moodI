...
import com.datatorrent.batch.EndApplicationControlTuple;
import com.datatorrent.io.fs.AbstractBatchFileInputOperator.BatchDirectoryScanner;
...
public class AbstractBatchFileInputOperatorTest
{
  ...
  public void testBatchIdempotencyWithCheckPoint() throws Exception
  {
    ...
    // emit start Batch and start batch emitted
    oper.beginWindow(0);
    oper.emitTuples();
    oper.endWindow();
    ...
    oper.emitTuples();
    oper.endWindow();
    List<String> beforeRecovery8 = Lists.newArrayList(queryResults.collectedTuples);
    List<ControlTuple> beforeRecoveryControlTuples8 = Lists.newArrayList(queryResults.collectedControlTuples);
    ...
    Assert.assertEquals("No control tuples emitted", 0, beforeRecoveryControlTuples7.size());

    oper.beginWindow(++startwid);
    oper.emitTuples();
    oper.endWindow();
    Assert.assertEquals("lines", beforeRecovery8, queryResults.collectedTuples);
    Assert.assertTrue("End Batch control tuple",
        beforeRecoveryControlTuples8.get(0) instanceof BatchControlTuple.EndBatchControlTuple);
  ...
  public void testBatchShutdown() throws Exception
  {
    ...
    Assert.assertTrue(controlTuplesBeforeRecovery.get(0) instanceof BatchControlTuple.StartBatchControlTuple);
    Assert.assertTrue(controlTuplesBeforeRecovery.get(1) instanceof BatchControlTuple.EndBatchControlTuple);
    Assert.assertTrue(controlTuplesBeforeRecovery.get(2) instanceof EndApplicationControlTuple);
    Assert.assertTrue(controlTuplesAfterRecovery.get(0) instanceof BatchControlTuple.StartBatchControlTuple);
    Assert.assertTrue(controlTuplesAfterRecovery.get(1) instanceof BatchControlTuple.EndBatchControlTuple);
    Assert.assertTrue(controlTuplesAfterRecovery.get(2) instanceof EndApplicationControlTuple);
  ...
  private static class DirectoryScannerNew extends BatchDirectoryScanner
  {
